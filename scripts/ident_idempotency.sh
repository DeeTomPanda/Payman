# 1. Create business
business_response=$(curl -s -X POST http://localhost:8080/businesses \
  -H "Content-Type: application/json" \
  -d '{"name": "Corp Idempotency"}')

echo "$business_response" | jq .

# save the api_key from response
export API_KEY=$(echo "$business_response" | jq -r '.api_key')

# 2. Create customer
export CUSTOMER_ID=$(curl -s -X POST http://localhost:8080/customers \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "Doe", "email": "doe@example.com"}' | jq -r '.id')

echo "Customer: $CUSTOMER_ID"

# 3. Create invoice
export INVOICE_ID=$(curl -s -X POST http://localhost:8080/invoices \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d "{
    \"customer_id\": \"$CUSTOMER_ID\",
    \"due_date\": \"2026-12-01\",
    \"line_items\": [
      {\"description\": \"Consulting\", \"quantity\": 2, \"unit_amount_cents\": 5000}
    ]
  }" | jq -r '.invoice.id')

echo "Invoice: $INVOICE_ID"

# 4. Finalize invoice (draft → open)
curl -s -X POST http://localhost:8080/invoices/$INVOICE_ID/finalize \
  -H "Authorization: Bearer $API_KEY" | jq .


# Below are the tests for same idempotency keys with different scenarios
# 5. Simulate retries with same idempotency key to return cached result if it fails
curl -s -X POST http://localhost:8080/payments/$INVOICE_ID/pay \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: pay-001" \
  -d '{"card_token": "tok_insufficient_funds"}' | jq .

# 6. Return the prevous response to avoid retries
curl -s -X POST http://localhost:8080/payments/$INVOICE_ID/pay \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: pay-001" \
  -d '{"card_token": "tok_success"}' | jq .

